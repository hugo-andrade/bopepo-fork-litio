...
class ViewerPDF {
	...
	private static URL TEMPLATE_PADRAO_COM_SACADOR_AVALISTA = getTemplate("file:///C:/tmp/pdf/BoletoTemplateComSacadorAvalista.pdf");
	private static URL TEMPLATE_PADRAO_SEM_SACADOR_AVALISTA = getTemplate("file:///c:/tmp/pdf/BoletoTemplateSemSacadorAvalista.pdf");
	...
	public static URL getTemplate(String template){
	 ...
	 * 
	 * @throws JRimumException Quando ocorrer um problema na geraÃ§Ã£o do PDF que estÃ¡ fora do controle
	 * da biblioteca.
	 * 
	 * @since 0.2
	 */
	protected static File groupInOnePDF(String pathName, List<Boleto> boletos, BoletoViewer boletoViewer) {
		...
		try {
			...
			log.error("Erro durante geraÃ§Ã£o do PDF." + e.getLocalizedMessage(), e);
			throw new JRimumException("Erro durante geraÃ§Ã£o do PDF. Causado por " + e.getLocalizedMessage(), e);
	...
	protected File getFile(String pathName) {
		...
		try {
			...
			log.error("Erro durante a criaÃ§Ã£o do arquivo. " + e.getLocalizedMessage(), e);
			throw new JRimumException("Erro durante a criaÃ§Ã£o do arquivo: [" + pathName + "]. " +
					"Causado por " + e.getLocalizedMessage(), e);
	...
	protected ByteArrayOutputStream getStream() {
		...
		try {
			...
			log.error("Erro durante a criaÃ§Ã£o do stream. " + e.getLocalizedMessage(), e);
			throw new JRimumException("Erro durante a criaÃ§Ã£o do stream. " +
					"Causado por " + e.getLocalizedMessage(), e);
	...
	/**
	 * <p>
	 * Verifica se o template que serÃ¡ utilizado virÃ¡ do resource ou Ã© externo,
	 * ou seja, se o usuÃ¡rio definiu ou nÃ£o um template.
	 * </p>
	 * 
	 * @return true caso o template que pode ser definido pelo usuÃ¡rio for null;
	 *         false caso o usuÃ¡rio tenha definido um template.
	 * 
	 * @since
	 */
	...
	private void setLogoBanco() throws MalformedURLException, IOException, DocumentException {
		...
		// AtravÃ©s da conta bancÃ¡ria serÃ¡ descoberto a imagem que representa o
		// banco, com base
		// no cÃ³digo do banco.
		...
		if (isNotNull(conta.getBanco().getImgLogo())) {
			...
			if (BancoSuportado.isSuportado(conta.getBanco().getCodigoDeCompensacaoBACEN().getCodigoFormatado())) {
				...
				if (isNotNull(imgLogoBanco)) {
					...
					// Esta imagem gerada aqui Ã© do tipo java.awt.Image
					conta.getBanco().setImgLogo(ImageIO.read(url));
				}

				// Se o banco em questÃ£o Ã© suportado nativamente pelo
				// componente,
				// entÃ£o um alerta serÃ¡ exibido.
				if (log.isDebugEnabled()) {
					log.debug("Banco sem imagem da logo informada. "
							+ "Com base no cÃ³digo do banco, uma imagem foi "
				...
				// Sem imagem, um alerta Ã© exibido.
	...
	/**
	 * Exibe os valores de instÃ¢ncia.
	 * 
